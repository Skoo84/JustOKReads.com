 ...
 * @param $userId
 */
function generateFreiChat($userId, $completeClient)
{
    ...
    $js = "
        import('{$freiChatBaseURL}/v1/freichat-mobile.js?pubKey=$publicKey&userId=$id&displayName=$name&displayImage=$avatar&change=$changeable&completeClient=$completeClient')";
    ...
    if($completeClient === "YES") {
        add_css("{$freiChatBaseURL}/static/v1/freichat-mobile.bundle.css?change=$changeable", array(
            'name' => 'freichat_css',
            'type' => 'remote'
        ));
...
Hook::add('before_site_head', function () {
    ...
    if (CurrentUser::loggedIn()) {
        ...
        if(!Store::has('client_loaded')) {
            generateFreiChat(CurrentUser::id(), "NO");
        }
    }
});
