 ...
 */
function add_css($asset, $options = false) {

    global $stream;
    $stream->addCSS($asset, $options);
}
...
/* function get_css() {
  ...
  return \CODOF\Theme\Css::get_css();
  }

  function remove_css($name) {

  return \CODOF\Theme\Css::remove_css($name);
  } */

/**
 * @param $asset
 * @param $options [type=>file|inline|defer|remote data=>data]
 */
 ...
 */
function add_js_body($asset, $options = false) {
 ...
 *//*
  ...
  return \CODOF\Theme\Js::get_js();
  }

  function remove_js($name) {

  return \CODOF\Theme\Js::remove_js($name);
  } */


function add_hook($hook, $function, $args = array(), $priority = 10) {

    CODOF\Hook::add($hook, $function, $args, $priority);
}

function call_hook($hook, $arg) {

    CODOF\Hook::call($hook, $arg);
}
